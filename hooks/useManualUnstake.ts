...
import {
  getStakePoolAccount,
  ValidatorListLayout,
  STAKE_POOL_PROGRAM_ID as StakePoolLibProgramId,
} from '@solana/spl-stake-pool';
import { getAssociatedTokenAddressSync, getAccount, TOKEN_PROGRAM_ID, TokenInstruction } from '@solana/spl-token';
  ...
  const unstake = async (amount: number): Promise<boolean> => {
    ...
    try {
      ...
      const minimumRequiredLamports =
        network === WalletAdapterNetwork.Testnet ? minimumRequiredLamports_Testnet : minimumRequiredLamports_Mainnet;
