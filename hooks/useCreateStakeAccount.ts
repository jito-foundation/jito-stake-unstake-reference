...
export const useCreateStakeAccount = () => {
  ...
  const createStakeAccount = async (amount: number): Promise<PublicKey | null> => {
    ...
    try {
      ...
      const poolTokensToReceive = Number(
        (BigInt(lamports) * BigInt(poolTokenSupply.toString())) / BigInt(totalLamports.toString())
      );

      console.log(`Will receive ${poolTokensToReceive} pool tokens from deposit`);

      // Get deposit instructions
      const { instructions: depositInstructions, signers: depositSigners } = await solanaStakePool.depositSol(
        connection as any,
        JITO_STAKE_POOL_ADDRESS,
        wallet.publicKey,
        lamports
      ...
      const validatorListData = solanaStakePool.ValidatorListLayout.decode(validatorListAccountInfo.data);
