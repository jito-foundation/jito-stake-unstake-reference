...
export const useManualStakeDeposit = () => {
  ...
  const depositStakeAccount = async (stakeAccountAddress: PublicKey): Promise<boolean> => {
    ...
    try {
      console.log(`Depositing stake account ${stakeAccountAddress.toString()} using manual stake deposit method...`);
      ...
      const [validatorStake] = PublicKey.findProgramAddressSync(
        [validatorVoteAccount.toBuffer(), JITO_STAKE_POOL_ADDRESS.toBuffer(), Buffer.alloc(0)],
        STAKE_POOL_PROGRAM_ID
      );
      ...
      const depositStakeAuthorityInfo = await connection.getAccountInfo(depositStakeAuthority);
      ...
      const [depositReceiptAddress] = PublicKey.findProgramAddressSync(
        [Buffer.from('deposit_receipt'), JITO_STAKE_POOL_ADDRESS.toBuffer(), base.publicKey.toBuffer()],
        STAKE_DEPOSIT_INTERCEPTOR_PROGRAM_ID
      );
