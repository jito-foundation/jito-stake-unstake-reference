...
export const useAssistedStakeDeposit = () => {
  ...
  const depositStakeAccount = async (stakeAccountAddress: PublicKey): Promise<boolean> => {
    ...
    try {
      console.log(`Depositing stake account ${stakeAccountAddress.toString()} using assisted stake deposit method...`);
      ...
      const [depositReceiptAddress] = PublicKey.findProgramAddressSync(
        [Buffer.from('deposit_receipt'), JITO_STAKE_POOL_ADDRESS.toBuffer(), baseKeypair.publicKey.toBuffer()],
        STAKE_DEPOSIT_INTERCEPTOR_PROGRAM_ID
      );
